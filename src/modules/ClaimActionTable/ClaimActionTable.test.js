...
import ClaimActionTable from './ClaimActionTable';
import ClaimActionRow from './ClaimActionRow';
import * as utils from 'utils';
import { MultiSelect, TableFilters } from 'components';

const renderClaimActionTable = () => {
  return render(<ClaimActionTable />);
};

describe('MODULES › ClaimAction › toggle filters', () => {
  renderClaimActionTable();
...
describe('MODULES › ClaimActionRow › Claim Action Row', () => {
  ...
  const renderClaimActionRow = (props={}) => {
    return render(
      <ClaimActionTable>
        <ClaimActionRow {...defaultProps} {...props} />
      </ClaimActionTable>);
  };

  describe('@render', () => {
    it('renders without crashing', () => {
      renderClaimActionRow();
    });

    it('renders with Claim Action Table', () => {
      //arrange
      const { queryByTestId } = renderClaimActionRow();

      //assert
      expect(queryByTestId('claim-action-table')).toBeInTheDocument();
