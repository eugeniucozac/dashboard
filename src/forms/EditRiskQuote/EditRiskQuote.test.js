...
import EditRiskQuote from './EditRiskQuote';
...
const quote = {
  ...
  premium: 100000,
  validUntil: '2021-03-26T13:44:04.932',
...
describe('FORMS › EditRiskQuote', () => {
  describe('@render', () => {
    it('renders without crashing', () => {
      // arrange
      const { container } = render(<EditRiskQuote risk={{ id: 1 }} />);
    ...
    it("renders nothing if there's no risk ID", () => {
      // arrange
      const { container } = render(<EditRiskQuote risk={{ foo: 1 }} />);
    ...
    it("renders nothing if there's no quote facility", () => {
      // arrange
      const { container } = render(<EditRiskQuote risk={{ id: 1 }} quote={{}} />);
    ...
    it('renders the form inputs', () => {
      // arrange
      const { container } = render(<EditRiskQuote risk={risk} quote={quote} />);

      // assert
      expect(screen.getByTestId('form-edit-risk-quote')).toBeInTheDocument();
      expect(screen.queryByText('app.cancel')).toBeInTheDocument();
      expect(screen.queryByText('risks.updateAndAccept')).toBeInTheDocument();
      expect(screen.queryByText('risks.update')).toBeInTheDocument();
      ...
      expect(screen.getByLabelText('risks.grossPremium')).toBeInTheDocument();
      expect(container.querySelector(getFormNumber('premium')).value).toBe('100,000');

      expect(screen.getByLabelText('form.dateValidUntil.label')).toBeInTheDocument();
      expect(container.querySelector(getFormDatepicker('validUntil')).value).toBe('26-03-2021');
